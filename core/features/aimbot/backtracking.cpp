...
float lerp_time() {
	int ud_rate = Interfaces::Console->Convar("cl_updaterate")->get_int();
	convar* min_ud_rate = Interfaces::Console->Convar("sv_minupdaterate");
	convar* max_ud_rate = Interfaces::Console->Convar("sv_maxupdaterate");

	if (min_ud_rate && max_ud_rate)
		ud_rate = max_ud_rate->get_int();

	float ratio = Interfaces::Console->Convar("cl_interp_ratio")->get_float();

	if (ratio == 0)
		ratio = 1.0f;

	float lerp = Interfaces::Console->Convar("cl_interp")->get_float();
	convar* c_min_ratio = Interfaces::Console->Convar("sv_client_min_interp_ratio");
	convar* c_max_ratio = Interfaces::Console->Convar("sv_client_max_interp_ratio");
...
bool backtracking::is_tick_valid(float simulation_time, float backtrack_time) {
	...
	std::clamp(correct, 0.f, Interfaces::Console->Convar("sv_maxunlag")->get_float());
...
void backtracking::store(c_usercmd* cmd) {
	if (!csgo::local_player || !Vars.backtracking.enabled || !Vars.backtracking.range)
		return;
	...
	for (int i = 1; i <= 64; i++) {
		...
		for (auto tick : records[i])
			if (!is_tick_valid(tick.simulation_time, float(Vars.backtracking.range) / 1000.f))
				records[i].pop_back();
	}
}
void backtracking::run(c_usercmd* cmd) {
	...
	for (int i = 1; i <= 64; i++) {
		...
		vec3_t view_angles{};
		math::angle_vectors((cmd->viewangles + (csgo::local_player->aim_punch_angle() * Interfaces::Console->Convar("weapon_recoil_scale")->get_float())), view_angles);
	...
	if (closest_player != -1) {
		for (int t = 0; t < records[closest_player].size(); t++) {
			vec3_t view_angles2{};
			math::angle_vectors((cmd->viewangles + (csgo::local_player->aim_punch_angle() * Interfaces::Console->Convar("weapon_recoil_scale")->get_float())), view_angles2);
