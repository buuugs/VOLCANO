...
void esp::render() {
	...
	if (Vars.misc.antiaim && Vars.misc.antiaim_arrows && csgo::local_player->is_alive() && csgo::aa_running && !Interfaces::input->m_fCameraInThirdPerson)
		aa_arrows();
	if (Vars.visuals.bomb_timer)
		bomb_timer();
	if (Vars.misc.speclist) 
		spectator_list();
		...
		if (i <= 64 && player->is_player() && player->is_alive() && player->team() != csgo::local_player->team()) {
			...
			if (ctx.pl) {
				...
				if (math::world_to_screen(ctx.pl->get_hitbox_position(hitbox_head), pl_head) || 
					...
					if (!Vars.pl_esp.dead_only || Vars.pl_esp.dead_only && !csgo::local_player->is_alive()) {
						if (Vars.pl_esp.key_bind == 0 || (GetKeyState(Vars.pl_esp.key_bind) && !Vars.pl_esp.dead_only)) {
							ctx.weapon_offset = 0;
							if (Vars.pl_esp.box) 
								box();
							if (Vars.pl_esp.name) 
							...
							if (Vars.pl_esp.ammo) 
								ammo();
							if (Vars.pl_esp.health) 
								health();
							if (Vars.pl_esp.weapon) 
								weapon();
							if (Vars.pl_esp.skeleton) 
								skeleton();
						}
					}
					if (Vars.misc.radar) 
...
void esp::box() {
	...
	render::draw_outline(ctx.bbox.x + 1, ctx.bbox.y + 1, ctx.bbox.w - 2, ctx.bbox.h - 2, color::black(ctx.outline_alpha[ctx.pl->index()]));
	render::draw_outline(ctx.bbox.x, ctx.bbox.y, ctx.bbox.w, ctx.bbox.h, color::from_float(Vars.pl_esp.box_clr, ctx.main_alpha[ctx.pl->index()]));
}
...
void esp::name() {
	...
	render::draw_text_string(ctx.bbox.x + ctx.bbox.w / 2, ctx.bbox.y - 13, render::fonts::esp_font, name, true, color::from_float(Vars.pl_esp.name_clr, ctx.main_alpha[ctx.pl->index()]));
}
...
void esp::weapon() {
	...
	if (weapon) {
		...
		if (weapon_data) {
			...
			render::draw_text_string(ctx.bbox.x + ctx.bbox.w / 2, ctx.bbox.y + ctx.bbox.h + 1 + ctx.weapon_offset, render::fonts::esp_font_small, weapon_name, true, color::from_float(Vars.pl_esp.weapon_clr, ctx.main_alpha[ctx.pl->index()]));
		}
	}
}
...
void esp::ammo() {
	...
	if (weapon) {
		...
		int bar_width = ((ctx.bbox.w - 2) * ammo) / weapon_data->m_iMaxClip;
		render::draw_filled_rect(ctx.bbox.x, ctx.bbox.y + ctx.bbox.h + 2, ctx.bbox.w, 4, color::black(ctx.outline_alpha[ctx.pl->index()]));
		render::draw_filled_rect(ctx.bbox.x + 1, ctx.bbox.y + ctx.bbox.h + 3, bar_width, 2, color::from_float(Vars.pl_esp.ammo_clr, ctx.main_alpha[ctx.pl->index()]));
	}
}
	...
	if (studio_model)
	{
		for (int j = 0; j < studio_model->bones_count; j++)
		{
			...
			if (bone && (bone->flags & BONE_USED_BY_HITBOX) && (bone->parent != -1))
			{
				...
				render::draw_line(s_parent.x, s_parent.y, s_child.x, s_child.y, color::from_float(Vars.pl_esp.skeleton_clr, ctx.main_alpha[ctx.pl->index()]));
			}
		}
	}
}
...
void esp::aa_arrows() {
	render::draw_text_string(ctx.screen_w / 2 - 40 - 6, ctx.screen_h / 2 - 12, render::fonts::arrow_font, "<", false, !csgo::aa_side ? color::white(255) : color::from_float(Vars.misc.desyncclr1, 255));
	render::draw_text_string(ctx.screen_w / 2 + 40 - 6, ctx.screen_h / 2 - 12, render::fonts::arrow_font, ">", false, csgo::aa_side ? color::white(255) : color::from_float(Vars.misc.desyncclr1, 255));
...
void esp::spectator_list() {
	...
	render::draw_text_string(4, ctx.screen_h / 2 - 80 - 15, render::fonts::arrow_font, "Spectators:", false, color::white(220));
}