...
using namespace bmenu::globals;
bmenu::functions_t bmenu::functions;
bmenu::gui_window_context_t bmenu::globals::window_ctx;

// ========================================================================
bool bmenu::button(const char *id, const vec2 size) {
...
// ========================================================================
bool bmenu::tab_button(const char* id, const vec2 size, const bool toggled, const unsigned long fontd) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
    ...
    if (toggled)
    {
        window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + size.y - 2}, zgui_render_type::zgui_filled_rect, global_colors.tab_active_or_clicked, "", {size.x - 2, 1} });
        window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + size.y - 3}, zgui_render_type::zgui_filled_rect, global_colors.tab_outline, "", {size.x - 2, 2} });
    }
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, global_colors.tab_idle, "", {size.x - 2, size.y - 2} });
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x, draw_pos.y}, zgui_render_type::zgui_filled_rect, global_colors.tab_outline, "", size });
...
// ========================================================================
void bmenu::checkbox(const char *id, bool &value) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::combobox(const char *id, std::vector<std::string> items, int &value, const int size) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
void bmenu::opencombobox(const char* id, std::vector<std::string> items, int& value, const int size) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 3, draw_pos.y + 1 }, zgui_render_type::zgui_text, global_colors.color_text, items.at(value), vec2{0, 0}, font });
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, global_colors.control_active_or_clicked, "", {float(control_width) - 2.f, control_height - 2} });
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x, draw_pos.y}, zgui_render_type::zgui_filled_rect, global_colors.control_outline, "", {float(control_width), control_height} });
...
// ========================================================================
void bmenu::multi_combobox(const char *id, std::vector<multi_select_item> items) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::colorpicker(const char* id, float* value, const float pusher)
{
	std::vector<std::string> id_split = utils::hash::split_str(id, '#');
	...
	if (const bool hovered = utils::input::mouse_in_region(draw_pos.x, draw_pos.y, control_width, control_height); hovered && utils::input::key_pressed(VK_LBUTTON)) {
		...
		bmenu::picker_slider(std::string("Red#" + id_split[0]).c_str(), 0.f, 1.f, value[0], value);
		bmenu::picker_slider(std::string("Green#" + id_split[0]).c_str(), 0.f, 1.f, value[1], value);
		bmenu::picker_slider(std::string("Blue#" + id_split[0]).c_str(), 0.f, 1.f, value[2], value);
...
// ========================================================================
void bmenu::begin_groupbox(std::string_view title, const vec2 size) {
	const unsigned long font = utils::misc::pop_font();
...
// ========================================================================
void bmenu::key_bind(const char *id, int &value) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::listbox(const char *id, std::vector<multi_select_item> items) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::slider_int(const char *id, const int min, const int max, int &value) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
void bmenu::picker_slider(const char* id, const int min, const int max, float& value, float* slidercol) {
	std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::slider_float(const char *id, const float min, const float max, float &value, const int decimales) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
bool bmenu::clickable_text(const char *id, const bool toggled) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::text(const char *text) {
    const unsigned long font = utils::misc::pop_font();
...
// ========================================================================
void bmenu::text_input(const char *id, std::string &value, const int max_length, const int flags) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void bmenu::toggle_button(const char *id, const vec2 size, bool &value) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
bool bmenu::begin_window(std::string_view title, const vec2 default_size, const unsigned long font, const int flags) {
	...
	if (window_ctx.opened || window_ctx.alpha > 0) {
        ...
        functions.draw_text((window_ctx.position.x + window_ctx.size.x / 2 - title.length() / 2), window_ctx.position.y + 2, global_colors.control_active_or_clicked, font, false, title.data());
...
// ========================================================================
void bmenu::next_column(const int pusher_x, const int pusher_y) {
    const vec2 cursor_pos = utils::misc::pop_cursor_pos();
...
void bmenu::fix_spacing(const int pusher_y) {
	const vec2 cursor_pos = utils::misc::pop_cursor_pos();
	utils::misc::push_cursor_pos(vec2{ cursor_pos.x, cursor_pos.y + pusher_y });
}
// ========================================================================
std::vector<std::string> bmenu::utils::hash::split_str(const char *str, const char separator) {
...
// ========================================================================
uint32_t bmenu::utils::hash::hash(const char *str, const uint32_t value) {
...
void bmenu::poll_input(std::string_view window_name) {
...
// ========================================================================
// Function for starting our input loop.
void bmenu::poll_input(HWND hwnd) {
   // if (!hwnd)
     //   throw std::exception("No window from where input should be read from specified in function parameter.");
...
// ========================================================================
bool bmenu::utils::input::key_pressed(const int key) {
    return key_state[key] && !prev_key_state[key];
}

// ========================================================================
bool bmenu::utils::input::key_down(const int key) {
    return key_state[key];
}

// ========================================================================
bool bmenu::utils::input::key_released(const int key) {
    return !key_state[key] && prev_key_state[key];
}

// ========================================================================
// Check if mouse is hovered over specified region.
bool bmenu::utils::input::mouse_in_region(const int x, const int y, const int w, const int h) {
...
void bmenu::utils::misc::push_cursor_pos(const vec2 pos) {
    window_ctx.cursor_pos.push(pos);
}

// ========================================================================
// Pop cursor position from the stack defined in window context.
bmenu::vec2 bmenu::utils::misc::pop_cursor_pos() {
...
void bmenu::utils::misc::push_font(const unsigned long font) {
    window_ctx.fonts.push(font);
}

// ========================================================================
// Push font to the stack defined in window context.
unsigned long bmenu::utils::misc::pop_font() {
