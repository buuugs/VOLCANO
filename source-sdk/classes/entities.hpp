...
public:
	...
	bool is_heavyr() {
		return (this->item_definition_index() == WEAPON_M249 ||
			this->item_definition_index() == WEAPON_NEGEV);
	}

	bool is_shotgun() {
		return (this->item_definition_index() == WEAPON_XM1014 ||
			this->item_definition_index() == WEAPON_MAG7 || 
			this->item_definition_index() == WEAPON_NOVA ||
			this->item_definition_index() == WEAPON_SAWEDOFF
				);
	}

	bool is_pm() {
		return (this->item_definition_index() == WEAPON_MAC10 ||
			this->item_definition_index() == WEAPON_P90 ||
			this->item_definition_index() == WEAPON_UMP45 ||
			this->item_definition_index() == WEAPON_MP9 ||
			this->item_definition_index() == WEAPON_BIZON ||
			this->item_definition_index() == WEAPON_MP7 ||
			this->item_definition_index() == WEAPON_MP5_SD ||
			this->item_definition_index() == WEAPON_MAC10);
	}
...
public:
	...
	void set_abs_angles(vec3_t angles)
	{
		...
		static fn abs_angles = (fn)(utilities::pattern_scan(GetModuleHandleA("client.dll"), "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8"));
	...
	void set_abs_origin(vec3_t origin)
	{
		...
		static fn abs_origin = (fn)utilities::pattern_scan(GetModuleHandleA("client.dll"), "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8");

		abs_origin(this, origin);
	}

	void invalidate_bone_cache() {
		static DWORD addr = (DWORD)utilities::pattern_scan(GetModuleHandleA("client.dll"), "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81");
	...
	void create_state(anim_state* state) {
		...
		static auto ret = (fn)utilities::pattern_scan(GetModuleHandleA("client.dll"), "55 8B EC 56 8B F1 B9 ? ? ? ? C7 46"); //55 8B EC 56 8B F1 B9 ? ? ? ? C7 46
		if (!ret)
			return;
	...
	void update_state(anim_state* state, vec3_t ang) {
		...
		static auto ret = reinterpret_cast<fn>(utilities::pattern_scan(GetModuleHandleA("client.dll"), "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24")); // 55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24
